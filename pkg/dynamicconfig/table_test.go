...
func TestWatchKey(t *testing.T) {
	...
	e, w := WatchKey(db.ReadTxn(), dct, key)

	if e.Key.Name != key || e.Value != value || e.Key.Source != lowPrioritySource {
		t.Errorf("Entry mismatch for key %v: expected (key=%v, source=%v, value=%v), but got (key=%v, source=%v, value=%v)", key, key, lowPrioritySource, value, e.Key, e.Key.Source, e.Value)
	}

	go waitFor(func() {
		<-w
		e, _ := WatchKey(db.ReadTxn(), dct, key)
		if e.Key.Name != key || e.Value != newValue || e.Key.Source != cmName {
			t.Errorf("Entry mismatch for key %v: expected (key=%v, source=%v, value=%v), but got (key=%v, source=%v, value=%v)", key, key, lowPrioritySource, value, e.Key, e.Key.Source, e.Value)
		}
	}, done)
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, cm := range tc.cms {
				for k, v := range cm.Data {
					obj, _, found := dct.Get(db.ReadTxn(), ByKeyIndex(Key{Name: k, Source: cm.Name}))
